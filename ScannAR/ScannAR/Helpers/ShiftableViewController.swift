...
class ShiftableViewController: UIViewController, UITextFieldDelegate, UITextViewDelegate, UIGestureRecognizerDelegate {
    ...
    @objc private func keyboardWillShow(notification: Notification) {
        ...
        if let textField = textFieldBeingEdited  {
            if self.children[0].view.frame.origin.y == 0 {
                ...
                self.children[0].view.frame.origin.y -= yShift
            }
    ...
    @objc private func keyboardWillHide(notification: Notification) {
        ...
        if self.children[0].view.frame.origin.y != 0 { self.children[0].view.frame.origin.y += currentYShiftForKeyboard }
    ...
    private func yShiftWhenKeyboardAppearsFor(textInput: UIView, keyboardSize: CGRect, nextY: CGFloat) -> CGFloat {
        ...
        let textFieldOrigin = self.children[0].view.convert(textInput.frame, from: textInput.superview!).origin.y
        ...
        let maximumY = self.children[0].view.frame.height - (keyboardSize.height + view.safeAreaInsets.bottom)
